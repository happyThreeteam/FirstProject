...
public class ManagerGangneung {
   public ArrayList<CategoryVO> categoryAllData()
    {
      ArrayList<CategoryVO> list=new ArrayList<CategoryVO>();
          ...
          for(int k=1;k<=10;k++)
          {
             try
             {
                Document doc=Jsoup.connect("http://www.mangoplate.com/search/%EA%B0%95%EB%A6%89%EC%8B%9C?keyword=%EA%B0%95%EB%A6%89%EC%8B%9C&page="+k).get();
   ...
   public static void main(String[] args) {
      // TODO Auto-generated method stub
	   ManagerGangneung fm=new ManagerGangneung();
      
   
      ArrayList<CategoryVO> list=fm.categoryAllData();
      try
      {
    	  File f=new File("c:\\data\\강릉카테고리.txt");
          if(f.exists())
             f.delete();

         FileWriter fw=new FileWriter("c:\\data\\강릉카테고리.txt");
      ...
      try
      {
    	  File f=new File("c:\\data\\강릉세부사항.txt");
          if(f.exists())
             f.delete();

         FileWriter fw=new FileWriter("c:\\data\\강릉세부사항.txt"); // true없으면 덮어쓰기
