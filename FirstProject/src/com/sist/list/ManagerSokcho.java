...
public class ManagerSokcho {
   public ArrayList<CategoryVO> categoryAllData()
    {
      ArrayList<CategoryVO> list=new ArrayList<CategoryVO>();
          ...
          for(int k=1;k<=10;k++)
          {
             try
             {
                Document doc=Jsoup.connect("http://www.mangoplate.com/search/%EC%86%8D%EC%B4%88%EC%8B%9C?keyword=%EC%86%8D%EC%B4%88%EC%8B%9C&page="+k).get();
   ...
   public static void main(String[] args) {
      ...
      ManagerSokcho fm=new ManagerSokcho();
      
   
      ArrayList<CategoryVO> list=fm.categoryAllData();
      try
      {
    	  File f=new File("c:\\data\\속초카테고리.txt");
          if(f.exists())
             f.delete();

         FileWriter fw=new FileWriter("c:\\data\\속초카테고리.txt");
      ...
      try
      {
    	  File f=new File("c:\\data\\속초세부사항.txt");
          if(f.exists())
             f.delete();

         FileWriter fw=new FileWriter("c:\\data\\속초세부사항.txt"); // true없으면 덮어쓰기
