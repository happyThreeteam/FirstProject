...
public class ManagerMapo{
   public ArrayList<CategoryVO> categoryAllData()
    {
      ArrayList<CategoryVO> list=new ArrayList<CategoryVO>();
          ...
          for(int k=1;k<=10;k++)
          {
             try
             {
                Document doc=Jsoup.connect("http://www.mangoplate.com/search/%EB%A7%88%ED%8F%AC%EA%B5%AC?keyword=%EB%A7%88%ED%8F%AC%EA%B5%AC&page="+k).get();
         ...
         for(int i=0;i<cList.size();i++)
         {
            ...
            for(int j=0;j<10;j++)
            {
               try
               {
                  ...
                  Element addr=doc2.select("tr td").get(0);
                  Element price=doc2.select(" tr td").get(3);
                  Element kind=doc2.select(" tr td").get(2);
                  Element hour=doc2.select(" tr td").get(5);
                  Element car=doc2.select(" tr td").get(4);
   ...
   public static void main(String[] args) {
      // TODO Auto-generated method stub
	   ManagerMapo fm=new ManagerMapo();
   
      ArrayList<CategoryVO> list=fm.categoryAllData();
      try
      {
    	  File f=new File("c:\\data\\마포카테고리.txt");
          if(f.exists())
             f.delete();
         FileWriter fw=new FileWriter("c:\\data\\마포카테고리.txt");
      ...
      try
      {
    	  File f=new File("c:\\data\\마포세부사항.txt");
          if(f.exists())
             f.delete();
         FileWriter fw=new FileWriter("c:\\data\\마포세부사항.txt"); // true없으면 덮어쓰기
